...
public static class MeshConversionUtility
{
    ...
    {
        _showConversionSettings = EditorGUILayout.Foldout(_showConversionSettings, "Gaussian Splat to Mesh Conversion Settings");
        ...
        if (_showConversionSettings)
        {
            ...
            var newMinDetailSizeValue = EditorGUILayout.Slider("Min Detail Size", GaussianSplattingPackageSettings.Instance.MinDetailSize, 0f, 1f);
            ...
            EditorGUI.BeginChangeCheck();
            var newSimplifyValue = EditorGUILayout.Slider("Simplify", GaussianSplattingPackageSettings.Instance.Simplify, 0f, 1f);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            EditorGUI.BeginChangeCheck();
            var newAngleLimit = EditorGUILayout.IntSlider("Angle Limit", GaussianSplattingPackageSettings.Instance.AngleLimit, 0, 360);
            if (EditorGUI.EndChangeCheck())
            {
    ...
    public static IEnumerator SendBytesToServer(byte[] inputData, string fileName, Action<byte[]> onResponseReceived, Action<string> onError = null)
    {
